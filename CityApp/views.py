...
DB_CONFIG = {
    ...
    'charset': 'utf8',
    'ssl': {
        'ssl_mode': 'REQUIRED'} if os.getenv(
        'DB_SSL',
...
def _send_complaint_update_email(complaint_id, subject, event_description):
    def email_task():
        try:
            ...
            with con:
                cur = con.cursor()
                # Get complaint details and citizen email in one join or
                # sequential calls
                cur.execute(
                    f"select description, citizenname FROM complaint where complaint_id='{complaint_id}'")
                comp_row = cur.fetchone()
                if comp_row:
                    desc, citizen_name = comp_row
                    cur.execute(
                        f"select email_id FROM signup where username='{citizen_name}'")
                    ...
                    if email_row and email_row[0]:
                        ...
                        print(
                            f"Email sent to {recipient_email} for complaint #{complaint_id}")
...
def predictDamage(path):
    ...
    with torch.no_grad():
        results = model.predict(
            frame, imgsz=320, conf=0.25, augment=False, verbose=False)
    ...
    if results and len(results) > 0 and results[0].boxes is not None:
        ...
        for data in detections:
            ...
            if float(confidence) >= CONFIDENCE_THRESHOLD:
                ...
                cv2.putText(frame, "Damage", (xmin, ymin),
        ...
        if result == 0:
            cv2.putText(frame, 'No Damage Detected', (50, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
    ...
    # Convert numpy buffer to bytes for base64 encoding
    ...
    if request.method == 'GET':
        ...
        if photo_filename:
            photo_path = os.path.join(
                settings.MEDIA_ROOT, 'photo', photo_filename)
        ...
        if db_cursor.rowcount == 1:
            ...
            _send_complaint_update_email(
                cid,
                f"SmartCity: Complaint #{cid} Deleted",
                "Your complaint has been removed from the system.")
            if uname == 'admin':
    ...
    if request.method == 'POST':
        try:
            ...
            if 't5' not in request.FILES:
                return render(
                    request, 'UserScreen.html', {
                        'data': '<div class="status-banner error">Please upload a photo.</div>'})
            ...
            try:
                ...
                _send_complaint_update_email(
                    ticket, f"SmartCity: Complaint #{ticket} Registered", event_desc)
            except Exception:
                pass
            ...
            return render(
                request, 'UserScreen.html', {
                    'data': f'<div class="status-banner error">Something went wrong: {str(e)}</div>'})
        ...
        with con:
            ...
            for row in rows:
                ...
                complaints_list.append({
                    ...
                    'cost': str(cost),
                    'desc': desc_str[:60] if len(desc_str) > 60 else desc_str,
                    'date': str(cdate)
                })
            ...
            resolved_count = statuses.get('Closed', 0)

            # Slice manually for type checkers
            recent_activity = []
            for i, item in enumerate(complaints_list):
                if i < 5:
